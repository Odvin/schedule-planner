...
export default class DailySchedule extends Schedule {
  ...
  private dailySchedule(): void {
    if (this.whichDay === 0) {
      this.dailyScheduleForEachWorkingDay();
    } else if (this.whichDay > 0 && this.whichDay < 7) {
      this.dailyScheduleForEachNthDay();
    } else {
      throw new Error('Error :: Incorrect schedule options. Days of the week for the task was not set.');
    }
  }
  ...
  private dailyScheduleForEachWorkingDay(): void {
    ...
    if (!this.schedulePeriod.length) {
      this.schedulePeriod = this.nextWeekWorkingDays();
    }
  }
  ...
  private dailyScheduleForEachNthDay(): void {
    ...
    if (!this.schedulePeriod.length) {
      this.schedulePeriod = this.nextWeekNthDays();
    }
  }
